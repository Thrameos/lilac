...
if [ -z "$JAVA_HOME" ]; then
	echo "Please set the variable JAVA_HOME to the java runtime's directory. You need at least JRE 1.8"
else 
	JASM_JAVA_CMD=$JAVA_HOME/bin/java
	if [ -f "$JASM_JAVA_CMD" ]; then 
		"$JASM_JAVA_CMD" -classpath "$JASM_INSTALL_DIR/*" org.jasm.tools.Disassembler $*
	else 
